...
export const GOOGLE_GMAIL_PROVIDER = "gmail";

/** Minimum scope to send emails on behalf of the connected user. */
export const GOOGLE_GMAIL_SCOPES = [
  "https://www.googleapis.com/auth/gmail.send",
];

export interface GoogleGmailOAuthTokenPayload {
  access_token: string;
...
/**
 * Returns a valid access token for Gmail API, refreshing and persisting if needed.
 */
export async function getGoogleGmailAccessToken(
  ...
  let payload: GoogleGmailOAuthTokenPayload;
  try {
    const raw = decryptToken(row.encrypted_tokens as string);
    payload = JSON.parse(raw) as GoogleGmailOAuthTokenPayload;
  } catch {
    return null;
  }
  ...
  if (needsRefresh && payload.refresh_token) {
    try {
      ...
      const newPayload: GoogleGmailOAuthTokenPayload = {
      ...
      await upsertIntegration(
        ...
        (row.scopes as string[]) ?? GOOGLE_GMAIL_SCOPES,
        encrypted
      );
      return newPayload.access_token;
    } catch (e) {
      console.error("Google Gmail token refresh failed:", e);
      return null;
    }
  }
