-- ============================================================
-- 00033_property_optioning_test_intake.sql
--
-- Enriquece el intake de `property_optioning` con campos inmobiliarios
-- mínimos para que el caso de prueba pueda alimentar tools como
-- easybroker_search_listings / easybroker_search_closed_deals en modo
-- "Datos del caso".
--
-- Primero llena el schema completo cuando todavía no existe `property_zone`.
-- Después parchea campos conocidos en schemas parcialmente actualizados (por
-- ejemplo versiones privadas que ya tienen `property_zone` pero conservan
-- operation/property_type como select).
-- ============================================================

update public.operational_case_types
set intake_schema_jsonb = jsonb_build_array(
  jsonb_build_object(
    'name', 'property_title',
    'label', 'Título / propiedad',
    'type', 'text',
    'required', true,
    'placeholder', 'Ej. Departamento Lomas Altas'
  ),
  jsonb_build_object(
    'name', 'owner_name',
    'label', 'Nombre del propietario',
    'type', 'text',
    'required', false,
    'placeholder', 'Nombre del dueño/lead'
  ),
  jsonb_build_object(
    'name', 'property_zone',
    'label', 'Zona / colonia',
    'type', 'text',
    'required', true,
    'placeholder', 'Ej. Lomas Altas, Zapopan'
  ),
  jsonb_build_object(
    'name', 'operation_type',
    'label', 'Operación aplicable',
    'type', 'multi_select',
    'required', true,
    'options', jsonb_build_array(
      jsonb_build_object('value', 'sale', 'label', 'Venta'),
      jsonb_build_object('value', 'rent', 'label', 'Renta')
    ),
    'help_text', 'Puedes seleccionar una o ambas opciones.'
  ),
  jsonb_build_object(
    'name', 'property_type',
    'label', 'Tipo de propiedad',
    'type', 'multi_select',
    'required', true,
    'options', jsonb_build_array(
      jsonb_build_object('value', 'Casa', 'label', 'Casa'),
      jsonb_build_object('value', 'Departamento', 'label', 'Departamento'),
      jsonb_build_object('value', 'Terreno', 'label', 'Terreno'),
      jsonb_build_object('value', 'Oficina', 'label', 'Oficina'),
      jsonb_build_object('value', 'Local', 'label', 'Local')
    ),
    'help_text', 'Puedes seleccionar todos los tipos aplicables.'
  ),
  jsonb_build_object(
    'name', 'target_price',
    'label', 'Precio objetivo (MXN)',
    'type', 'number',
    'required', false,
    'min', 0,
    'step', 1,
    'unit', 'MXN',
    'placeholder', 'Ej. 5500000',
    'help_text', 'Por ahora se interpreta en pesos mexicanos. Soporte multi-moneda (ej. USD) queda como mejora futura.'
  ),
  jsonb_build_object(
    'name', 'area_m2',
    'label', 'Superficie / construcción m²',
    'type', 'number',
    'required', false,
    'min', 0,
    'step', 1,
    'placeholder', 'Ej. 180',
    'help_text', 'Se usa para proponer un rango comparable ±20%.'
  ),
  jsonb_build_object(
    'name', 'bedrooms',
    'label', 'Recámaras',
    'type', 'number',
    'required', false,
    'min', 0,
    'step', 1,
    'placeholder', 'Ej. 3'
  ),
  jsonb_build_object(
    'name', 'bathrooms',
    'label', 'Baños',
    'type', 'number',
    'required', false,
    'min', 0,
    'step', 0.5,
    'placeholder', 'Ej. 2.5',
    'help_text', 'Permite medios baños.'
  ),
  jsonb_build_object(
    'name', 'parking_spaces',
    'label', 'Estacionamientos',
    'type', 'number',
    'required', false,
    'min', 0,
    'step', 1,
    'placeholder', 'Ej. 2'
  ),
  jsonb_build_object(
    'name', 'telegram_chat_id',
    'label', 'Telegram chat_id externo',
    'type', 'number',
    'required', false,
    'min', 0,
    'step', 1,
    'placeholder', 'Ej. 123456789',
    'help_text', 'Opcional; requiere que el contacto haya escrito al bot.'
  )
)
where case_type = 'property_optioning'
  and not exists (
    select 1
    from jsonb_array_elements(intake_schema_jsonb) as field
    where field->>'name' = 'property_zone'
  );
