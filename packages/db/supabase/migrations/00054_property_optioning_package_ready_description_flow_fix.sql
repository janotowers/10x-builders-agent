-- ============================================================
-- 00054_property_optioning_package_ready_description_flow_fix.sql
--
-- Repara package_ready cuando 00053 no actualizó skill_tools (p. ej. flujo
-- ya parcialmente customizado). Idempotente: no-op si analyze_property_images
-- ya está registrado. Preserva required_assets del watermark existente.
-- ============================================================
...
with package_ready_targets as (
  ...
  where oct.case_type = 'property_optioning'
    ...
    and not exists (
      select 1
      from jsonb_array_elements(coalesce(skill_pos.skill->'skill_tools', '[]'::jsonb)) tool
      where tool->>'tool_id' = 'analyze_property_images'
    )
)
