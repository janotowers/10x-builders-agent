-- ============================================================
-- 00053_property_optioning_package_ready_description_flow.sql
--
-- Actualiza el contrato UI/readiness de package_ready en property_optioning
-- para incluir:
-- - análisis de imágenes
-- - enriquecimiento de entorno
-- - preparación de borrador de descripción
-- - publicación por destino con aprobación separada
--
-- Usa jsonb_set por índice (mismo patrón que 00043+) para no depender de
-- reemplazar el array completo del flujo.
-- ============================================================
  ...
  operational_flow_jsonb = jsonb_set(
    jsonb_set(
      jsonb_set(
        ...
        array[targets.step_idx::text, 'step_label'],
        '"Gestionar publicación"'::jsonb,
        false
      ),
      array[targets.step_idx::text, 'step_description'],
      '"Valida prerequisitos, prepara descripción comercial aprobada y publica por destino con HITL."'::jsonb,
      false
    ),
    ...
    jsonb_build_array(
      ...
      jsonb_build_object(
        ...
        'tool_description', 'Solicita revisión/aprobaciones internas y emite el resumen final de cierre (listing_published_summary).'
      ),
      jsonb_build_object(
