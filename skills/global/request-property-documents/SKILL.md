...
guardrails: |
  ...
  hasta tener evidencia de cada documento solicitado en el contexto.
---
...
- **Predial** (último pago).
- **Escritura** (primera y última hoja basta para verificación).
- **Identificación oficial** del propietario (anverso/reverso).
...
1. Lee el caso. Identifica:
   ...
   - `context_jsonb.required_documents`: si no existe, usa la lista
     default de arriba.
   - `context_jsonb.documents_received`: array de documentos ya en mano.
...
2. Si **aún no se ha mandado el primer mensaje** (no hay evento
   ...
   a. Compón un mensaje cordial en español pidiendo TODOS los documentos
      pendientes, formateado como bullet list. Incluye breve frase de
...
4. Si llegó respuesta del externo (`external_response`):
   ...
   b. Compara contra `required_documents`. Si están todos:
      ...
      - manda `notify_user("Documentos completos para caso X.")`.

   c. Si faltan algunos: manda mensaje cortés agradeciendo lo que llegó
      y pidiendo solo los que faltan. Mantén `waiting_external` y
      reprograma `next_action_at`.
...
• Escritura (primera y última hoja)
