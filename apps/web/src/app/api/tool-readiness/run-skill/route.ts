...
function buildSkillTestMessage(params: {
  ...
  if (params.skill.skill_slug === "perform-comparable-analysis") {
    lines.push(
      ...
      "Si Avaclick responde validation_error con missing_required_fields, registra warning y continúa con EasyBroker/BigQuery (no bloquees el paso por esa fuente). Si operational_case_persist_comparables_analysis no da muestra defendible (defensible_sample=false / unique_comparable_count < 3; p. ej. usable_count=0): NO avances a price_proposal_pending; deja current_step=comparables_in_progress y status=waiting_internal; ejecuta notify_user al asesor con datos de la propiedad, filtros usados y sugerencias concretas para ampliar búsqueda (precio, m², meses).",
      "Si devuelve defensible_sample=true (unique_comparable_count >= 3): el sistema avanza por la ruta determinística a price_proposal_pending; notify_user al asesor. No uses telegram_send_message_to_contact."
    );
  }
