...
- **Main agent + conversation classifier default models**: code fallbacks for
  ...
  constant). Same classifier env still drives intake routing and the
  pending-decision HITL `unclear` second opinion. README + `.env.example` +
  model-role docs updated; README links
