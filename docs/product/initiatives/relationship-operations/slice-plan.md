...
> **Version:** v1.3  
> **Status:** Approved — governing R1 Slice Plan; SL-1 **READY**, planned into Cycle 1, and **Executable** — case-B prerequisite satisfied 2026-09-03  
...
- **Dependencies:**
  ...
  - **Alebrixe legacy read identities — dependency case B** (Methodology §10.2, v0.3.2): controlled by our team, with the concrete prerequisite contract stated below, and satisfiable before this Slice starts. Classified first-hand on 2026-09-03; re-evaluated against the corrected rule.
    ...
    **Current provisioning scope — revalidate before credential issuance; not a semantic capability contract.** The physical names below reflect the 2026-09-03 source topology and are allowed to drift behind the bounded capability contract, which is what TD-5 makes durable. They are recorded so the identities can be scoped concretely, not to promote legacy physical topology into architecture.
    ...
    - *Mongo:* the **`appointments`** collection in the primary database (`MONGO_DB_NAME`) only.
    ...
    **Revalidated 2026-09-03 before issuance** (sequenced item 1). Both legacy repositories had moved again; the drift was physical only — a log-severity change, an anti-upsert guard on a *write* path, and one additive query helper — with no read-topology change. Two scope corrections came out of it:
    ...
    Confirmed with the legacy owner that `bot` is what `MONGO_DB_NAME` resolves to, so the delivered grant does reach `appointments`. Also confirmed from source that `property_data` lives in a *different* database (`MONGO_DB_NAME_V1`), which the delivered identity does not reach — the exclusion holds at the credential perimeter, not only in code.
...
**Prerequisite satisfied — 2026-09-03.** All four sequenced items are complete, so SL-1's case-B dependency is met and the Slice is Executable (Planned, confirmed Accountable, prerequisite satisfied, capacity available — nothing else is in flight).
...
| 4 — secret material available | Present to the authorized setup path via the git-ignored local environment file; the Firestore key is referenced **by file path**, so no key material sits inside an environment value. No secret value appears in Git, logs, PRs, fixtures or evidence |

**Accepted deviation from least privilege (Mongo).** The delivered identity grants database-level `read` on two databases (`bot`, `gu2`) rather than a collection-level `find` on `appointments` alone. `gu2` holds reminder, template-retry, ads and bug-informer collections that SL-1 never queries, and `bot` also carries the conversation store this Slice deliberately reads from Firestore instead. The security-critical property holds — **read-only, no write authority, single cluster** — and the residual exposure is materially smaller than the project-wide Firestore read already accepted under TD-5. Narrowing further was judged not worth a round trip now, partly because elaboration has not yet established which collections in `bot` the appointment capability touches, and an over-tight grant is its own failure mode. **Revisit before Gate B.**

**Production key deliberately not wired.** The `ungga-full` key is issued but intentionally not referenced by the setup path until the hosted evidence run for SA-1.2/SA-1.3, which is the only step that needs a real Alebrixe lead. This keeps the window in which a project-wide production Firestore reader is active as short as TD-5's time-boxed framing intends. Wiring it is a step inside the Slice, not a new prerequisite.
...
| v1.3 | 2026-09-03 | **Cycle 1 case-B prerequisite completed; SL-1 becomes Executable.** Scope revalidated before issuance, as sequenced item 1 required: both legacy repositories had moved again, but the drift was physical only. Two corrections resulted — Firestore appointments live under `deals/{deal_id}/appointments` rather than in a root collection, and `users_sellers` is dropped because SL-1 resolves Organization binding through Gu OS `external_identity_bindings`. Confirmed with the legacy owner that `bot` is `MONGO_DB_NAME`, and from source that `property_data` sits in a different database the delivered identity cannot reach, so that exclusion holds at the credential perimeter. Dedicated read-only identities issued on both sides; secret material is available to the authorized setup path through the git-ignored local environment file, with the Firestore key referenced by path so no key material sits in an environment value. The production Firestore key is issued but deliberately unwired until the hosted evidence run. One deviation from least privilege is accepted and recorded for review before Gate B: the Mongo grant is database-level on `bot` and `gu2` rather than collection-level on `appointments`; it remains read-only, single-cluster, and materially smaller than the project-wide Firestore read TD-5 already accepts. `.gitignore` now covers the key and password material, matching the existing convention of keeping secrets in the working tree and out of history. No implementation, no JIT Tasks. |
