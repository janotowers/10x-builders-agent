...
> **Version:** v1.5  
> **Status:** Approved — governing R1 Slice Plan; SL-1 **READY**, planned into Cycle 1, **Executable** since 2026-09-04, and **in execution with RS-2 hosted evidence complete** (2026-09-05). **Not Done**: deterministic CI evidence is still outstanding. See [`sl1-hosted-evidence.md`](sl1-hosted-evidence.md)  
...
The correction also makes the deviation load-bearing rather than merely tolerable: a grant confined to `bot` would not reach the appointment source at all. If the deviation is ever narrowed *before* its retirement condition — which the revalidation does not require — the minimal grant is a collection-level `find` on **`gu2.appointments`**.
