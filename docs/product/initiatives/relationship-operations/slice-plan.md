...
> **Version:** v1.2  
> **Status:** Approved — governing R1 Slice Plan; SL-1 **READY** and planned into Cycle 1, not yet Executable (case-B credential prerequisite unprovisioned)  
...
- **Dependencies:**
  ...
  - **Alebrixe legacy read identities — dependency case B** (Methodology §10.2, v0.3.2): controlled by our team, with the concrete prerequisite contract stated below, and satisfiable before this Slice starts. Classified first-hand on 2026-09-03; re-evaluated against the corrected rule.
    ...
    **Current provisioning scope — revalidate before credential issuance; not a semantic capability contract.** The physical names below reflect the 2026-09-03 source topology and are allowed to drift behind the bounded capability contract, which is what TD-5 makes durable. They are recorded so the identities can be scoped concretely, not to promote legacy physical topology into architecture.
    ...
    - *Firestore:* the Lead record and its `wsp_messeges` subcollection (including `asesor_<phone>` threads), `users` and `users_sellers` for owner/Organization resolution, `properties`, and the replicated appointment record.
    - *Mongo:* **the appointment operational record only.**
...
| SL-1 | Cycle 1 | Alejandro Torres Padilla | 3–5 days / Medium *(frozen at READY, 2026-09-03)* | — | — | — | — | — | RS-2 → — | — |
...
**Cycle 1 — sequenced plan.** Target length approximately one week. The case-B prerequisite is sequenced **first**, inside the Cycle, as named prerequisite / setup work. Under Section 18 proportionality it is deliberately **not** a Slice of its own: issuing two scoped read identities is configuration, not Slice-sized work.

| Order | Work | Kind |
|---|---|---|
| 1 | Issue the Firestore / GCP read identity with the approved scopes, no write authority | prerequisite / setup |
| 2 | Issue the Mongo Atlas read identity, read-only, scoped to the appointment operational record | prerequisite / setup |
| 3 | Revalidate the current provisioning scope (§4) against live legacy topology before issuance | prerequisite / setup |
...
**How provisioning time is accounted.** Time spent on items 1–4 is **not** part of SL-1's 3–5 day agent-assisted engineering elapsed. It is recorded separately as **human / external wait**, and it also counts toward **calendar elapsed**, per Methodology §17.1. Folding it into the engineering figure would corrupt the first calibration data point this initiative produces.
...
| v1.2 | 2026-09-03 | **First Execution Cycle planning decision.** SL-1 confirmed into **Cycle 1** (~1 week) with **Alejandro Torres Padilla** as confirmed human Accountable / DRI — the two facts that together constitute `Planned`. Only SL-1 was eligible: it is the sole READY Slice, SL-0 is historical and SL-2…SL-13 are NOT READY stubs. Per Methodology §12.1 the case-B prerequisite is **explicitly sequenced first**, inside the Cycle, as named prerequisite / setup work; under §18 proportionality it is deliberately not a Slice of its own. Provisioning time is accounted as **human / external wait** and **calendar elapsed**, never inside SL-1's 3–5 day agent-assisted engineering estimate, which stays frozen. If provisioning slips far enough to make the Cycle target unrealistic, the Cycle is re-planned **explicitly** rather than the estimate or the Cycle being quietly extended. **SL-1 is Planned but not Executable**: the two external read identities do not exist yet. No JIT Tasks, no implementation, no credentials created. Following §19.2, the register records the Cycle and DRI but never the execution stage. |
