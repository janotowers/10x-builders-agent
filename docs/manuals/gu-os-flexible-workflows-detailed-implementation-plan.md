...
**Tasks:**
- [x] 1. Definition-version selector: soporte API completo — `POST /api/operational-case-tests` acepta `workflow_definition_id` opcional (drafts propios o globales del case_type; valida pertenencia), regenerar **re-pinnea** a la definición resuelta (cierra el hueco de casos lab con pin null u obsoleto), y `createOperationalCase` acepta pin explícito (`workflowDefinition`). Dropdown UI en N0 (`operational-case-types-client.tsx` + opciones desde `listWorkflowDefinitionsVisibleToUser`) — ticket 1.6-1 cerrado (2026-08-04).
...
**Pendiente (ticket explícito):** ~~1.6-1 dropdown~~ cerrado (2026-08-04).
...
- [x] 3. Valuation verifier como `registered_specialized_worker` (antes `specialized_agent`; rename de taxonomía 2026-08-06, migración 00073) (`packages/agent/src/operational-cases/valuation-verifier.ts` + executor `registered-specialized-worker.ts`): contexto aislado = comparable set + hechos de la propiedad + precios propuestos, **jamás** el razonamiento de la recomendación; dos capas (checks deterministas de banda/consistencia + revisión opcional model-backed read-only); verdict fail ⇒ `requiresHumanReview` (gate del artefacto `price_recommendation`); perfil seed con `role: valuation_verifier`, `model_alias: reasoning_standard` (upgrade a `reasoning_high` cuando los contadores lo disparen).
...
**Status:** [x] done (2026-08-04) — tarea 5 cerrada (continuación al agente de `unmatched_intent`)
...
- [x] 3. El residual de 0.1 sigue siendo la red de seguridad: cero manejados ⇒ `{handled:false}` y el turno COMPLETO cae al agente como hoy; el residual compuesto reutiliza la misma línea "No actué sobre: …" de los adaptadores de canal cuando NO hay continuación al agente.
...
- [x] 5. Continuación al agente de intents-pregunta diferidos (`unmatched_intent`) en el mismo turno: tras el ack compuesto, Telegram re-despacha el texto residual al flujo del agente (segundo mensaje) y el chat web compone ack + respuesta del agente en una sola HTTP response. Solo `unmatched_intent` continúa; `unparsed_remainder` sigue con "No actué sobre …". Helper `deferredAgentContinuationText` + aserción B1 en `decomposed-turn.selftest.ts`.
...
**Phase 4 exit checks:** [x] A1/A2/B1/B2/D selftests pass (4.1-5: continuación al agente de `unmatched_intent` cableada en web + Telegram) · [~] a non-engineer creates/forks, validates, simulates, and publishes a simple workflow that runs on a synthetic case (capa scripts: `apps/web/scripts/studio-walkthrough.ts` fork→validate/simulate→evidence→publish opcional→caso sintético; **falta el walkthrough humano en UI** — checklist consolidado en `docs/manuals/gu-os-studio-human-walkthrough.md`, findings 25–26) · [x] publication is evidence-gated + human-approved (la action re-corre gates + registra evidencia en la misma llamada) · [x] capability map surfaces missing `required_assets` as customer-worded gaps linking to the assets panel · [x] Studio has absorbed the 2.7 shell (not rebuilt — catálogo/recursos son pestañas del mismo surface) · [ ] settings lab authoring retired (primera release hecha: aviso de deprecación + nav consolidada; retiro total espera cobertura de edición de plantillas en el Studio) **and** 2.7 assets panel was already shipping ✓ · [ ] if Slice 4.4 was activated, its cross-channel scenarios pass without silent/cross-tenant association · [ ] channel-linked views (§28.12) are not an exit requirement unless activated.
