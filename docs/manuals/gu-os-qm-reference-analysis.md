...
5. **Knowledge scoping follows the same tenant-first rule.** If multi-seat activates, the future Brain Layer's knowledge needs the scope dimension `workflow_definitions` already reserved (personal — an asesor's leads; team; org — zone dossiers, brokerage playbooks), nested strictly inside the tenant. Prior art: QM's scope-owned `memory` primitive and its "company brain" connectors, and G Brain v0.40+ company brain (federated sources + per-person OAuth). Recorded as a forward-looking note in [`gbrain-evaluation-and-plan.md`](../brain/gbrain-evaluation-and-plan.md) §1.4.8 — no brain schema exists yet, and none should be added for this before activation. **Canonical semantics now live in [`knowledge-scope-and-ownership.md`](./knowledge-scope-and-ownership.md)** (with [`ADR-101`](../adr/ADR-101-organization-tenancy.md) / [`ADR-102`](../adr/ADR-102-knowledge-ownership-scopes.md)): QM's `org/team/group/personal` scope kinds map to Gu's `organization/team/user`, but Gu adds two dimensions QM does not have — curated read-only `platform`/`industry` scopes, and the external identity bridge (`external_org_id`, `super-admin`/`vendedor` → memberships/roles). QM remains within-tenant prior art only; it is not a source for platform/industry curation or external-identity mapping.
...
- **`docs/brain/gbrain-evaluation-and-plan.md` (v1.6):** owns the knowledge/Brain boundary this document touches — `case_facts` as the preferred selective-promotion source into `brain_pages` (§1.4.8), the asset-classes × knowledge table, and the multi-seat knowledge-scoping note that pairs with §6 here (now expanded by §1.5.8–1.5.10: hybrid pipeline, ownership scopes, regenerable artifacts). The Brain program is sequenced after the workflows phases; nothing in this analysis accelerates it.
- **`knowledge-scope-and-ownership.md` + `ai-native-loops.md` + `docs/adr/` (2026-08-02):** the canonical ownership-scope semantics, loop contract/DRI/improvement-authority model, and decision records (ADR-100…105) that §6 and §7 here previously anticipated. Where this analysis describes QM behavior that overlaps those topics, the canonical documents govern Gu OS design; this analysis stays evidence about QM.
