...
| 1.7-8 | Skills: composite `property-optioning-coach` + atómicas (`request-property-documents`, `extract-property-characteristics`, `perform-comparable-analysis`, `prepare-listing-price`, `prepare-commission-contract`, `request-property-photos`, `publish-listing-package`). Inicialmente globales para desarrollo; al activar el piloto real se mueven a `account_skills` de Alebrixe. |
...
Temporal/Inngest, multi-agente, overrides granulares por tipo/canal/contacto en UI
(más allá del V1 por audiencia/kind), rama completa
