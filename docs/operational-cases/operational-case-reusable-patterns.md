...
| **Mappers destino** | EasyBroker: `commission_pct` → `operations[].commission`; colaboración → `share_commission` + `shared_commission_percentage` (solo 50\|null). Ungga: `commission_pct` → Comisión (%) en modal Operación; % colaborador no se mapea. % incompatible → warning, nunca muta canónico; override auditable en `publication.destinations.<destino>.commercial_override` |
