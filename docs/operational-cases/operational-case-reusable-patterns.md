...
| **Comportamiento** | **Paso probado** (`tested_ok`) sólo cuando **todos** los escenarios del registry tienen run exitoso por `scenario_id`. Progreso parcial: `partially_tested` + `step_test_progress` («2 de 4 escenarios probados»). Habilidad probada y escenarios pendientes: `awaiting_n4` (UI: **Falta probar escenarios del paso**). Sin escenarios N4: basta habilidad + integraciones probadas. Copy sin jerga N3/N4 en pills. |
