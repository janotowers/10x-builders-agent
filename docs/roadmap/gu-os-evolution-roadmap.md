...
7. **Trigger organization/multi-seat work from deployment reality.** Do not bundle organization tenancy with unrelated multi-agent/router work.
...
| **R1 — Relationship Operations v1 / absorb Traditional Gu responsibility** | Make Gu responsible for advancing a lead opportunity toward the best achievable next outcome, especially visit progression, rather than only responding/following fixed rules. | Define Lead Opportunity Case semantics; map Traditional Gu lead/conversation/matching/follow-up commitments into durable state/events; replace fixed timer follow-up with situational/event-driven next work; human involvement for relationship-sensitive moments; portfolio supervision projection; preserve WhatsApp-first prospect semantics. | In a pilot tenant, Gu can keep an opportunity alive across sessions/events, choose/execute allowed next work, escalate appropriately, and link progression to visit request/attended-visit evidence without manual pipeline operation. | **Next product increment** |
...
| **R3 — Organization / multi-seat model when product rollout requires it** | Make the brokerage, not one login, the durable collaboration/authorization boundary where real customers need shared operation. | Organizations, memberships, roles/teams only as justified; organization-owned Cases/knowledge/skills/integrations; assignment vs approval vs DRI remain distinct; migration from current profile/account assumptions. | A real multi-user brokerage can share governed work without cross-tenant leakage or collapsing team/role/DRI semantics. | **Evidence-gated enabler** |
...
4. **Introduce R3 organization/multi-seat only when a pilot/broad-rollout requirement makes the current profile/account model a blocker.**
...
- Whether organization/multi-seat is a blocker for the first Relationship Operations pilot or only for broader rollout.
